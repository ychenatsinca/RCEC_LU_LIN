...
source("fun_regrid_tccip_d03.R")

ld_go <- FALSE

if (ld_go) {
#load tccip d03 geo data
d03.fname = "./geo_em.d03.nc"
d03.geo <- fun_read_nc(arg1=d03.fname)

mask_arr_a  <- d03.geo$LANDMASK
nav_lon_a   <- d03.geo$CLONG
nav_lat_a   <- d03.geo$CLAT
...
lcfrac <- d03.geo$LANDUSEF
lcdom  <- d03.geo$LU_INDEX
# set resolution at 9km
dx=0.01
dy=0.01
...
if (ld.create.nc) {
  ...
  dim3 <- ncdim_def("west_east","",  1:330, create_dimvar=FALSE) 
  dim4 <- ncdim_def("south_north","",1:501, create_dimvar=FALSE)
  dim5 <- ncdim_def("south_north_stag","",1:502, create_dimvar=FALSE)
  dim6 <- ncdim_def("west_east_stag","",1:331,   create_dimvar=FALSE) 
  ...
  #==== assign file name ===
  out_filename <- paste("d03.geo_RCEC_",work.year,".nc",sep="") 
  ...
  org.data <- fun_read_nc(arg1=d03.fname,arg2=FALSE)
